    ...
    <Card className="life-story-card mb-8 animate-fade-in">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        <div className="stat-item">
          ...
          <div className="stat-label">Recording{stats.totalRecordings !== 1 ? 's' : ''}</div>
        </div>
        ...
        {hasMultipleRecordings && (
          <>
            <div className="stat-item">
              ...
              <div className="stat-value">{stats.recordingSpan}</div>
              <div className="stat-label">Days Span</div>
            </div>
          ...
          <div className="timeline-section">
            ...
            <div className="flex justify-between text-sm text-muted-foreground">
              <div>
                ...
                <div>{formatDate(stats.firstRecording)}</div>
              </div>
              <div className="text-right">
                <div className="font-medium">Latest</div>
                <div>{formatDate(stats.latestRecording)}</div>
              </div>
            </div>
          </div>
        </>
      ...
      {/* Themes */}
      {summary.themes && Object.keys(summary.themes).length > 0 && (
        <>
          <Separator className="my-6" />
          <div>
            <h3 className="text-sm font-medium text-foreground mb-3 flex items-center gap-2">
            ...
            <div className="flex flex-wrap gap-2">
              {Object.entries(summary.themes).map(([key, value]) => (
                <Badge
                  key={key}
                  variant="secondary"
                  className={getThemeColor(value)}
                >
                  {getThemeLabel(key)}: {value}
                </Badge>
              ))}
            </div>
          </div>
